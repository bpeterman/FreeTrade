    ...
    public class StockDrone // drone class. this will be our worker bees
    {
        ...
        public void dispatchFunction(object oFunction)//paramatized threads can only have objects as parameters
        {
            ...
            if (sFunction.Length >= 4)
            {
                ...
                switch (temp) //switch statments are faster than if and elses
                {
                    case 'A':
                    ...
                    case 'C':
                        if (temp2 == 'E')
                        {
                            temp2 = sFunction.ToUpper().ElementAt(sFunction.Length - 2);
                            //getChange
                            if (temp2 == 'G')
                            {
                                foreach (string s in stockSymbols)
                            ...
                            else
                            {
                                foreach (string s in stockSymbols)
                                {
                                    results.Add(new QueryResults(s, 0.0, stock.getCompanyName(s), "getCompanyName", true));
                                }
                            }
                        }
                        else if(temp2 == 'T')
